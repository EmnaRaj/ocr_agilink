"""Aggregated analytics computed from the (corrected) data.

Everything is derived from each fiche's raw_extraction so it reflects user
edits/validations, not the original relational snapshot. Extracted here as a
plain function so both the `/analytics` route and the chatbot's `get_overview`
...
def compute_overview(db: Session) -> dict:
    """Dashboard/agent overview: KPIs + breakdowns derived from raw_extraction."""
    fiches = db.scalars(
        select(Fiche)
        .options(
